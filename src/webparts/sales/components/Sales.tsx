...
export default class Form extends React.Component<ISalesProps, IFormState> {
  ...
  private loadProducts(): void {
    this.props.spHttpClient
      .get(`${this.props.siteUrl}/_api/web/lists/getbytitle('Products')/items?$select=ProductID,ProductName,ProductType,ProductExpirydate,ProductUnitPrice`, SPHttpClient.configurations.v1)
      .then((response: SPHttpClientResponse) => {
        if (response.ok) {
          response.json().then((data) => {
  ...
  private handleProductChange = (event: React.ChangeEvent<HTMLSelectElement>): void => {
    ...
    if (product) {
      this.setState({
        ...
        selectedExpiryDate: product.ProductExpirydate,
    ...
    return (
      <div className={styles.form}>
        ...
        <div>
          ...
          <input type="text" value={this.state.selectedProductType} readOnly />
        </div>
        <br></br>
        <div>
          <label>Product Expiry Date:</label>
          <input type="text" value={this.state.selectedExpiryDate} readOnly />
        </div>
        <br></br>
        <div>
          <label>Product Unit Value:</label>
          <input type="number" value={this.state.selectedUnitPrice} readOnly />
        </div>
        <br></br>
        <div>
        ...
        <div>
          ...
          <input type="text" value={this.state.selectedUnitPrice * this.state.unitsSold} readOnly />
        </div>
        <br></br>
        <div>
        ...
        <br></br>
        <div>
          <button className={styles.button} onClick={this.handleAddOrder}>Add Order</button>
          <button className={styles.button} onClick={this.handleEditOrder}>Edit Order</button>
          <button className={styles.button} onClick={this.handleDeleteOrder}>Delete Order</button>
